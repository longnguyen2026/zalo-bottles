...
set +e
# Wine may emit the harmless winemenubuilder diagnostic on stderr while
# starting ZaloSetup.exe. Filter ONLY that diagnostic; keep all real errors.
"${CLI[@]}" run -b "$BOTTLE" -e "$INSTALLER"     2> >(sed '/winemenubuilder\.exe -a -r/d' >&2)
ZALO_RC=$?
set -e
